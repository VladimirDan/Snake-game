...
const gameField = {
  ...
  randomEmptyCellCoords() {
    ...
    return gameField.cells[coords].item === items.none ? 
      coords : 
      gameField.randomEmptyCellCoords();
  },
...
const snake = {
  ...
  lose() {
	...
	const snakeHeadY = Math.floor(snake.snakeHead / gameField.fieldWidth);
	const purposeHeadY = Math.floor(purpose / gameField.fieldWidth);
  	const purposeIsOutX = purposeHeadY !==  snakeHeadY;
    ...
    const snakeIsBitingItSelf = gameField.cells[purpose].item === items.snake &&
      purpose !== snake.snakeBody[0];
